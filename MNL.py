...
def random_walk(N, Xo, m, a, rNums):
    #----------multiplicative linear congruential generator----#
    rNums1 = mu_li_co_ge(Xo, a, m, N)
    # scale them to [0,1]
    rNums1 = [num / (m-1) for num in rNums1]
    rNums1 = np.array(rNums1)
    # scale them to [0,2*pi]
    rNums1 = [num*2*math.pi for num in rNums1]
    dx = 0.0
    dy = 0.0
    x = [0]
    y = [0]
    ...
    R = 0
    for i in range(0, N):
        # pick up any random number from the array
        th = rNums1[int(random.uniform(0, N))]
        dx += math.cos(th)
        dy += math.sin(th)
